...
function Btn({children,onClick,v="primary",sm,full,disabled,style:sx}) {
  ...
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      fontFamily:"'DM Sans',system-ui,sans-serif", transition:"all .15s", ...sx,
    }}>{children}</button>
  );
}
...
function Field({label,value,onChange,type="text",placeholder,options,rows,required}) {
  const T = useT();
  const base = {width:"100%",padding:"9px 12px",borderRadius:9,border:`1.5px solid ${T.border}`,fontSize:13,fontFamily:"inherit",background:T.inputBg,color:T.text,outline:"none",boxSizing:"border-box"};
  return (
    <div style={{marginBottom:13}}>
      ...
      {options
        ? <select value={value} onChange={e=>onChange(e.target.value)} style={base}><option value="">— choose —</option>{options.map(o=><option key={o.v||o} value={o.v||o}>{o.l||o}</option>)}</select>
        : rows
...
function SearchBar({value,onChange,placeholder="Search…"}) {
  ...
  return (
    <div style={{position:"relative",flex:1,minWidth:180}}>
      ...
      {value && <button onClick={()=>onChange("")} style={{position:"absolute",right:9,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:T.muted,fontSize:16,lineHeight:1,padding:0}}>×</button>}
    </div>
  );
}
...
function Card({children,style}) {
  const T = useT();
  return <div style={{background:T.card,borderRadius:14,border:`1px solid ${T.border}`,padding:18,...style}}>{children}</div>;
}
...
function DarkToggle({dark,setDark}) {
  return (
    <button onClick={()=>setDark(d=>!d)} title={dark?"Light mode":"Dark mode"} style={{
      ...
      padding:"4px 6px",borderRadius:8,lineHeight:1,flexShrink:0,
    }}>
      {dark?"☀️":"🌙"}
    </button>
  );
      ...
      <div style={{padding:collapsed?"14px 10px":"18px 14px 14px",borderBottom:"1px solid rgba(255,255,255,.06)",display:"flex",alignItems:"center",justifyContent:collapsed?"center":"space-between",gap:10}}>
        {!collapsed && (
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:34,height:34,borderRadius:10,background:T.teal,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>🧺</div>
            <div>
        ...
        {collapsed && <div style={{width:34,height:34,borderRadius:10,background:T.teal,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>🧺</div>}
        {!collapsed && <button onClick={()=>setCollapsed(true)} style={{background:"rgba(255,255,255,.07)",border:"none",borderRadius:7,padding:"4px 8px",cursor:"pointer",color:"#94a3b8",fontSize:11,flexShrink:0}}>◀</button>}
      </div>
      {collapsed && <button onClick={()=>setCollapsed(false)} style={{background:"none",border:"none",color:"#94a3b8",cursor:"pointer",padding:"8px 0",fontSize:14,marginTop:4}}>▶</button>}

      <nav style={{flex:1,padding:collapsed?"8px 6px":"10px 8px",overflowY:"auto"}}>
        {NAV.map(n=>{
          const on=page===n.key;
          return (
            <button key={n.key} onClick={()=>go(n.key)} title={collapsed?n.label:""} style={{
              width:"100%",display:"flex",alignItems:"center",gap:collapsed?0:10,
              padding:collapsed?"10px":"10px 12px",borderRadius:10,border:"none",
              background:on?T.teal:"transparent",color:on?"#fff":T.sidebarText,
              ...
              {!collapsed && n.badge && pendingCount>0 && <span style={{background:T.orange,color:"#fff",fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:20}}>{pendingCount}</span>}
              {collapsed && n.badge && pendingCount>0 && <span style={{position:"absolute",top:6,right:6,width:8,height:8,borderRadius:"50%",background:T.orange}}/>}
            </button>
          );
        })}
      </nav>
...
function Orders({orders,setOrders}) {
  ...
  const filtered = useMemo(()=>{
    ...
    if(search.trim()){const q=search.toLowerCase();list=list.filter(o=>o.id.toLowerCase().includes(q)||o.customer.toLowerCase().includes(q)||o.phone.includes(q)||o.address.toLowerCase().includes(q));}
  ...
  const advance=o=>{setOrders(p=>p.map(x=>x.id===o.id?{...x,status:next[o.status]}:x));setOpen(prev=>prev?{...prev,status:next[o.status]}:null);};
  const cancel =o=>{setOrders(p=>p.map(x=>x.id===o.id?{...x,status:"Cancelled"}:x));setOpen(null);};
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:10}}>
        <div><h2 style={{fontSize:20,fontWeight:800,color:T.text,margin:0}}>Orders</h2><p style={{color:T.muted,fontSize:13,marginTop:4}}>{orders.length} total · {filtered.length} shown</p></div>
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          <SearchBar value={search} onChange={setSearch} placeholder="Search orders…"/>
          <SelectFilter value={sortBy} onChange={setSortBy}><option value="newest">Newest</option><option value="highest">Highest ₵</option><option value="lowest">Lowest ₵</option></SelectFilter>
        </div>
      </div>
      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        {tabs.map(t=>{const cnt=t==="All"?orders.length:orders.filter(o=>o.status===t).length;const on=tab===t;return <button key={t} onClick={()=>setTab(t)} style={{padding:"6px 14px",borderRadius:20,border:`1.5px solid ${on?T.teal:T.border}`,background:on?T.teal:T.card,color:on?"#fff":T.muted,fontSize:12,fontWeight:on?700:400,cursor:"pointer",fontFamily:"inherit",transition:"all .15s"}}>{t} <span style={{opacity:.7,fontSize:11}}>({cnt})</span></button>;})}
      </div>
      <Card style={{padding:0,overflow:"hidden"}}>
        {filtered.length===0?<EmptyState icon="📭" msg="No orders match your search"/>:(
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr style={{background:T.bgAlt}}>{["Order","Customer","Items","Total","Status","Paid","Action"].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead>
            <tbody>
              {filtered.map((o,i)=>(
                <tr key={o.id} style={{borderTop:`1px solid ${T.border}`,background:i%2?T.bgAlt:T.card}}>
                  ...
                  <td style={td}><div style={{fontWeight:600,color:T.text}}>{o.customer}</div><div style={{fontSize:11,color:T.muted}}>{o.phone}</div></td>
                  ...
                  <td style={td}><div style={{fontSize:12,color:o.paid?T.green:T.red,fontWeight:700}}>{o.paid?"✓ Paid":"✗ Unpaid"}</div><div style={{fontSize:11,color:T.muted}}>{o.method}</div></td>
        ...
        <Modal title={`Order ${open.id}`} onClose={()=>setOpen(null)} w={520}>
          <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:18,flexWrap:"wrap"}}>
            {["Pending","Packing","Delivering","Delivered"].map((s,i,arr)=>{
              ...
              const ci=all.indexOf(open.status);const ti=all.indexOf(s);
              const done=ci>=ti&&open.status!=="Cancelled";const isCur=open.status===s;
              return(
          ...
          <div style={{background:T.bgAlt,borderRadius:10,padding:14,marginBottom:16}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0 0",borderTop:`1px solid ${T.border}`,marginTop:4,fontWeight:800,fontSize:15,color:T.text}}><span>Total</span><span style={{color:T.teal}}>₵{open.subtotal+open.fee}</span></div>
          </div>
...
function Products({products,setProducts,categories}) {
  ...
  const handleImg=async e=>{const file=e.target.files[0];if(!file)return;const d=await readFile(file);setPreview(d);setF(x=>({...x,image:d}));};
  const save=()=>{
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:10}}>
        <div><h2 style={{fontSize:20,fontWeight:800,color:T.text,margin:0}}>Products</h2><p style={{color:T.muted,fontSize:13,marginTop:4}}>{products.length} items · {shown.length} shown</p></div>
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",gap:2,background:T.bgAlt,borderRadius:9,padding:3,border:`1px solid ${T.border}`}}>
            {["grid","list"].map(v=>(
              <button key={v} onClick={()=>setView(v)} style={{padding:"5px 9px",borderRadius:7,border:"none",background:viewMode===v?T.card:"transparent",cursor:"pointer",fontSize:13,color:viewMode===v?T.text:T.muted,boxShadow:viewMode===v?"0 1px 4px rgba(0,0,0,.15)":"none",transition:"all .15s"}}>{v==="grid"?"⊞":"☰"}</button>
            ))}
          </div>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        {["All",...categories.map(c=>String(c.id))].map(id=>{
          const c=categories.find(x=>String(x.id)===id);const on=catFilter===id;
          return <button key={id} onClick={()=>setCat(id)} style={{padding:"6px 13px",borderRadius:20,border:`1.5px solid ${on?T.teal:T.border}`,background:on?T.teal:T.card,color:on?"#fff":T.muted,fontSize:12,fontWeight:on?700:400,cursor:"pointer",fontFamily:"inherit"}}>{c?`${c.icon} ${c.name}`:"All"}</button>;
        })}
      </div>

      {shown.length===0?<Card><EmptyState icon="🔍" msg="No products match your filters"/></Card>:(
        viewMode==="grid"?(
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))",gap:14}}>
            {shown.map(p=>{
              const c=categories.find(x=>x.id===p.catId);const sl=stockLabel(p.stock);
              return(
          ...
          <Card style={{padding:0,overflow:"hidden"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead><tr style={{background:T.bgAlt}}>{["Product","Category","Price","Stock","Status","Action"].map(h=><th key={h} style={{padding:"10px 14px",textAlign:"left",fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:.5}}>{h}</th>)}</tr></thead>
              <tbody>
                {shown.map((p,i)=>{
                    ...
                    <tr key={p.id} style={{borderTop:`1px solid ${T.border}`,background:i%2?T.bgAlt:T.card}}>
                      <td style={{padding:"10px 14px"}}>
                        <div style={{display:"flex",alignItems:"center",gap:10}}>
                          ...
                          <div><div style={{fontWeight:600,color:T.text}}>{p.name}</div><div style={{fontSize:11,color:T.muted}}>{p.description}</div></div>
                        </div>
                      </td>
        ...
        <Modal title={editing?"Edit Product":"Add Product"} onClose={()=>{setOpen(false);setEdit(null);}}>
          <div style={{marginBottom:14}}>
            ...
            <div onClick={()=>fileRef.current.click()} style={{width:"100%",height:140,borderRadius:12,border:`2px dashed ${T.border}`,background:T.bgAlt,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",overflow:"hidden"}}>
              {preview?<img src={preview} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<div style={{textAlign:"center",color:T.muted}}><div style={{fontSize:28,marginBottom:5}}>📷</div><div style={{fontSize:12,fontWeight:600}}>Click to upload</div><div style={{fontSize:11}}>JPG, PNG, WEBP</div></div>}
            </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <div>
              ...
              <select value={f.catId} onChange={e=>setF(x=>({...x,catId:Number(e.target.value)}))} style={{width:"100%",padding:"9px 12px",borderRadius:9,border:`1.5px solid ${T.border}`,fontSize:13,fontFamily:"inherit",background:T.inputBg,color:T.text,outline:"none"}}>
                <option value="">— choose —</option>{categories.map(c=><option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}
              </select>
            </div>
...
function Categories({categories,setCategories,products}) {
  ...
  const shown=useMemo(()=>{if(!search.trim())return categories;const q=search.toLowerCase();return categories.filter(c=>c.name.toLowerCase().includes(q));},[categories,search]);
  const save=()=>{if(!f.name.trim())return;setCategories(p=>editing?p.map(c=>c.id===editing.id?{...c,...f}:c):[...p,{...f,id:Date.now()}]);setF(blank);setOpen(false);setEdit(null);};
  const remove=id=>{if(products.find(p=>p.catId===id))return alert("Remove all products in this category first.");if(window.confirm("Delete category?"))setCategories(p=>p.filter(c=>c.id!==id));};

  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:10}}>
        <div><h2 style={{fontSize:20,fontWeight:800,color:T.text,margin:0}}>Categories</h2><p style={{color:T.muted,fontSize:13,marginTop:4}}>{categories.length} categories</p></div>
        <div style={{display:"flex",gap:8}}><SearchBar value={search} onChange={setSearch} placeholder="Search categories…"/><Btn onClick={()=>{setF(blank);setEdit(null);setOpen(true);}}>+ Add Category</Btn></div>
      </div>
      {shown.length===0?<Card><EmptyState icon="🗂️" msg="No categories found"/></Card>:(
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12}}>
          {shown.map(c=>{const count=products.filter(p=>p.catId===c.id).length;return(
        ...
        <Modal title={editing?"Edit Category":"Add Category"} onClose={()=>{setOpen(false);setEdit(null);}}>
          <div style={{marginBottom:14}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
              {EMOJIS.map(e=><button key={e} onClick={()=>setF(x=>({...x,icon:e}))} style={{width:38,height:38,borderRadius:9,border:`2px solid ${f.icon===e?T.teal:T.border}`,background:f.icon===e?T.tealLt:T.bgAlt,fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>{e}</button>)}
            </div>
          </div>
...
function Customers({customers,setCustomers,orders}) {
  ...
  const shown=useMemo(()=>{
    ...
    if(search.trim()){const q=search.toLowerCase();list=list.filter(c=>c.name.toLowerCase().includes(q)||c.email.toLowerCase().includes(q)||c.phone.includes(q));}
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:10}}>
        <div><h2 style={{fontSize:20,fontWeight:800,color:T.text,margin:0}}>Customers</h2><p style={{color:T.muted,fontSize:13,marginTop:4}}>{customers.length} registered · {shown.length} shown</p></div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          <SearchBar value={search} onChange={setSearch} placeholder="Search customers…"/>
          <SelectFilter value={statusFlt} onChange={setStatus}><option value="All">All Status</option><option value="Active">Active</option><option value="Inactive">Inactive</option></SelectFilter>
          <SelectFilter value={sortBy} onChange={setSortBy}><option value="spent">Most Spent</option><option value="orders">Most Orders</option><option value="name">Name A–Z</option></SelectFilter>
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
        {[{label:"Total",value:customers.length,color:T.teal},{label:"Active",value:customers.filter(c=>c.status==="Active").length,color:T.green},{label:"Inactive",value:customers.filter(c=>c.status==="Inactive").length,color:T.muted}].map(s=>(
          <Card key={s.label} style={{textAlign:"center",padding:"14px 12px"}}><div style={{fontSize:26,fontWeight:800,color:s.color}}>{s.value}</div><div style={{fontSize:12,color:T.muted,marginTop:2}}>{s.label}</div></Card>
        ))}
      </div>
      {shown.length===0?<Card><EmptyState icon="👥" msg="No customers match your search"/></Card>:(
        <Card style={{padding:0,overflow:"hidden"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr style={{background:T.bgAlt}}>{["Customer","Phone","Orders","Spent","Last Seen","Status",""].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead>
            <tbody>
              {shown.map((c,i)=>(
                <tr key={c.id} style={{borderTop:`1px solid ${T.border}`,background:i%2?T.bgAlt:T.card}}>
                  <td style={{padding:"10px 14px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:9}}>
                      ...
                      <div><div style={{fontWeight:600,color:T.text}}>{c.name}</div><div style={{fontSize:11,color:T.muted}}>{c.email}</div></div>
                    </div>
                  </td>
      ...
      {open&&(
        <Modal title="Customer Profile" onClose={()=>setOpen(null)} w={500}>
          <div style={{textAlign:"center",marginBottom:18}}>
            <div style={{width:60,height:60,borderRadius:18,background:T.tealLt,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:20,color:T.teal,margin:"0 auto 10px"}}>{open.name.split(" ").map(n=>n[0]).join("").slice(0,2)}</div>
          ...
          <div style={{display:"flex",gap:8}}>
            <Btn full v={open.status==="Active"?"danger":"primary"} onClick={()=>{setCustomers(p=>p.map(c=>c.id===open.id?{...c,status:c.status==="Active"?"Inactive":"Active"}:c));setOpen(prev=>({...prev,status:prev.status==="Active"?"Inactive":"Active"}));}}>{open.status==="Active"?"🚫 Suspend Customer":"✅ Re-activate Customer"}</Btn>
            <Btn v="ghost" onClick={()=>setOpen(null)}>Close</Btn>
          </div>
        </Modal>
      )}
...
function Payments({orders}) {
  ...
  const byMethod= ["MTN MoMo","Card","Vodafone"].map(m=>({m,n:paid.filter(o=>o.method===m).length,total:paid.filter(o=>o.method===m).reduce((s,o)=>s+o.subtotal+o.fee,0),pct:paid.length?Math.round((paid.filter(o=>o.method===m).length/paid.length)*100):0}));
  const shown   = useMemo(()=>{let list=orders;if(methodFlt!=="All")list=list.filter(o=>o.method===methodFlt);if(search.trim()){const q=search.toLowerCase();list=list.filter(o=>o.id.toLowerCase().includes(q)||o.customer.toLowerCase().includes(q));}return list;},[orders,search,methodFlt]);

  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div><h2 style={{fontSize:20,fontWeight:800,color:T.text,margin:0}}>Payments</h2><p style={{color:T.muted,fontSize:13,marginTop:4}}>Revenue & transaction history</p></div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>
        {[{label:"Total Collected",value:`₵${revenue.toLocaleString()}`,icon:"💰",color:T.teal},{label:"Paid Orders",value:paid.length,icon:"✅",color:T.green},{label:"Avg. Order",value:`₵${paid.length?Math.round(revenue/paid.length):0}`,icon:"📊",color:T.orange}].map(s=>(
          <Card key={s.label} style={{display:"flex",alignItems:"center",gap:14}}>
            ...
            <div><div style={{fontSize:22,fontWeight:800,color:T.text,lineHeight:1}}>{s.value}</div><div style={{fontSize:12,color:T.muted,marginTop:3}}>{s.label}</div></div>
          </Card>
        ))}
      </div>
      <Card>
          ...
          <div key={m} style={{marginBottom:12}}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:5}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{height:5,borderRadius:4,background:T.bgAlt,overflow:"hidden"}}><div style={{height:"100%",width:`${pct}%`,background:T.teal,borderRadius:4}}/></div>
              </div>
            </div>
          </div>
        ))}
      ...
      <Card style={{padding:0,overflow:"hidden"}}>
        <div style={{padding:"12px 16px",borderBottom:`1px solid ${T.border}`,display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <SelectFilter value={methodFlt} onChange={setMethod}><option value="All">All Methods</option><option value="MTN MoMo">MTN MoMo</option><option value="Card">Card</option><option value="Vodafone">Vodafone</option></SelectFilter>
        </div>
        {shown.length===0?<EmptyState icon="💸" msg="No transactions match"/>:(
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr style={{background:T.bgAlt}}>{["Order","Customer","Amount","Method","Status"].map(h=><th key={h} style={{padding:"9px 14px",textAlign:"left",fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:.5}}>{h}</th>)}</tr></thead>
            <tbody>
              {shown.map((o,i)=>(
                <tr key={o.id} style={{borderTop:`1px solid ${T.border}`,background:i%2?T.bgAlt:T.card}}>
...
function Settings({onLogout,dark,setDark}) {
  ...
  const changePw=()=>{if(!curPw||!newPw){setPwMsg("Fill both fields.");return;}if(curPw!=="picky2024"){setPwMsg("Current password is wrong.");return;}setPwMsg("Password changed ✓");setCurPw("");setNewPw("");setTimeout(()=>setPwMsg(""),2500);};

  return (
    <div style={{display:"flex",flexDirection:"column",gap:16,maxWidth:540}}>
      <div><h2 style={{fontSize:20,fontWeight:800,color:T.text,margin:0}}>Settings</h2><p style={{color:T.muted,fontSize:13,marginTop:4}}>Store & admin configuration</p></div>
      ...
      <Card>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 14px",background:T.bgAlt,borderRadius:10,border:`1px solid ${T.border}`}}>
          ...
          <button onClick={()=>setDark(d=>!d)} style={{width:48,height:26,borderRadius:13,border:"none",cursor:"pointer",background:dark?T.teal:"#cbd5e1",transition:"background .2s",position:"relative",padding:0,flexShrink:0}}>
      ...
      <Card>
        ...
        {pwMsg&&<div style={{fontSize:12,color:pwMsg.includes("✓")?T.green:T.red,padding:"6px 10px",background:pwMsg.includes("✓")?T.greenL:T.redL,borderRadius:8,marginBottom:8}}>{pwMsg}</div>}
...
function Login({onLogin,dark}) {
  ...
  const go=()=>{if(!email||!pass){setErr("Enter email and password.");return;}setLoading(true);setErr("");setTimeout(()=>{if(email==="admin@pickybasket.com"&&pass==="picky2024")onLogin();else{setErr("Wrong email or password.");setLoading(false);}},600);};
    ...
    <div style={{minHeight:"100vh",background:dark?"linear-gradient(135deg,#040a10 60%,#0d2420)":"linear-gradient(135deg,#0f1923 60%,#1f7a6e)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:T.card,borderRadius:22,padding:"38px 34px",width:"100%",maxWidth:400,boxShadow:"0 32px 80px rgba(0,0,0,.5)",border:`1px solid ${T.border}`}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{width:60,height:60,borderRadius:18,background:T.tealLt,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,margin:"0 auto 14px",border:`2px solid ${T.teal}44`}}>🧺</div>
