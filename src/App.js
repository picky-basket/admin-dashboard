...
const statusStyle = s => ({
  ...
  "Pending":     {bg:T.yellowL, fg:"#b45309"},
...
const Btn = ({children,onClick,v="primary",sm,full,disabled,style:sx}) => (
...
const Field = ({label,value,onChange,type="text",placeholder,options,rows,required}) => (
  <div style={{marginBottom:13}}>
    ...
    {options ? (
      ...
      <textarea value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} rows={rows} style={{width:"100%",padding:"9px 12px",borderRadius:9,border:`1.5px solid ${T.border}`,fontSize:13,fontFamily:"inherit",background:T.white,color:T.text,outline:"none",resize:"vertical",boxSizing:"border-box"}}/>
    ) : (
      <input type={type} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} style={{width:"100%",padding:"9px 12px",borderRadius:9,border:`1.5px solid ${T.border}`,fontSize:13,fontFamily:"inherit",background:T.white,color:T.text,outline:"none",boxSizing:"border-box"}}/>
    )}
  </div>
);
...
const SearchBar = ({value,onChange,placeholder="Search…"}) => (
  <div style={{position:"relative",flex:1,minWidth:180}}>
    ...
    <input value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder}
      style={{width:"100%",padding:"8px 12px 8px 32px",borderRadius:9,border:`1.5px solid ${T.border}`,fontSize:13,fontFamily:"inherit",background:T.white,color:T.text,outline:"none",boxSizing:"border-box"}}/>
...
const Modal = ({title,onClose,children,w=480}) => (
  <div onClick={e=>{if(e.target===e.currentTarget)onClose();}} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
    <div style={{background:T.white,borderRadius:18,width:"100%",maxWidth:w,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 32px 80px rgba(0,0,0,.25)"}}>
      <div style={{padding:"16px 20px",borderBottom:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,background:T.white,zIndex:1,borderRadius:"18px 18px 0 0"}}>
        <span style={{fontWeight:800,fontSize:15,color:T.text}}>{title}</span>
        <button onClick={onClose} style={{background:T.bg,border:"none",width:28,height:28,borderRadius:8,fontSize:18,cursor:"pointer",color:T.muted,lineHeight:1,display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
      </div>
...
function Sidebar({page,go,pendingCount,collapsed,setCollapsed}) {
  return (
    <aside style={{width:collapsed?64:210,background:T.navy,display:"flex",flexDirection:"column",height:"100vh",position:"sticky",top:0,flexShrink:0,transition:"width .2s ease",overflow:"hidden"}}>
      <div style={{padding:collapsed?"14px 10px":"18px 14px 14px",borderBottom:"1px solid rgba(255,255,255,.07)",display:"flex",alignItems:"center",justifyContent:collapsed?"center":"space-between",gap:10}}>
        {!collapsed && (
        ...
        <button onClick={()=>setCollapsed(c=>!c)} style={{background:"rgba(255,255,255,.08)",border:"none",borderRadius:7,padding:"4px 7px",cursor:"pointer",color:"#94a3b8",fontSize:12,flexShrink:0,display:collapsed?"none":"flex",alignItems:"center"}}>
          ◀
        </button>
      </div>
      {collapsed && (
        <button onClick={()=>setCollapsed(false)} style={{background:"none",border:"none",color:"#94a3b8",cursor:"pointer",padding:"8px 0",fontSize:14,marginTop:4}}>▶</button>
      )}
        ...
        {NAV.map(n=>{
          ...
          return (
            <button key={n.key} onClick={()=>go(n.key)} title={collapsed?n.label:""} style={{
              ...
              background:on?T.teal:"transparent",color:on?"#fff":"#94a3b8",
        ...
        <div style={{padding:"12px 14px",borderTop:"1px solid rgba(255,255,255,.07)"}}>
          <div style={{display:"flex",alignItems:"center",gap:9}}>
            ...
            <div>
              ...
              <div style={{fontSize:10,color:"#94a3b8"}}>admin@pickybasket.com</div>
            </div>
          </div>
        </div>
      )}
...
function Dashboard({orders,products,customers,go}) {
  ...
  const orderCounts = statOrder.map(s=>({s,n:orders.filter(o=>o.status===s).length}));
    ...
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14}}>
        {[
          ...
          <Card key={s.label} style={{display:"flex",alignItems:"center",gap:14,cursor:"default"}}>
            <div style={{width:46,height:46,borderRadius:13,background:s.color+"1a",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>{s.icon}</div>
            <div>
            ...
            return (
              <div key={s} style={{marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}}>
                  ...
                  <span style={{color:T.muted}}>{n} order{n!==1?"s":""} · {pct}%</span>
                </div>
          ...
          {orders.slice(0,5).map((o,i)=>(
            <div key={o.id} style={{display:"flex",alignItems:"center",gap:12,padding:"8px 0",borderTop:i>0?`1px solid ${T.border}`:"none"}}>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontWeight:600,fontSize:13,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{o.customer}</div>
                <div style={{fontSize:11,color:T.muted}}>{o.id} · {o.time}</div>
              </div>
              <div style={{fontWeight:700,fontSize:13,whiteSpace:"nowrap"}}>₵{o.subtotal+o.fee}</div>
              <Tag s={o.status}/>
            </div>
          ))}
        </Card>
      ...
      {lowStock>0 && (
        <Card style={{borderLeft:`4px solid ${T.red}`,background:T.redL}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:8}}>
            {products.filter(p=>p.stock<=5).map(p=>(
              <div key={p.id} style={{display:"flex",justifyContent:"space-between",padding:"7px 10px",background:T.white,borderRadius:9,fontSize:13,border:`1px solid #fca5a5`}}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:10}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          <select value={sortBy} onChange={e=>setSortBy(e.target.value)} style={{padding:"8px 10px",borderRadius:9,border:`1.5px solid ${T.border}`,fontSize:12,fontFamily:"inherit",background:T.white,color:T.text,outline:"none",cursor:"pointer"}}>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        {tabs.map(t=>{
          ...
          const on = tab===t;
          return (
            <button key={t} onClick={()=>setTab(t)} style={{padding:"6px 14px",borderRadius:20,border:`1.5px solid ${on?T.teal:T.border}`,background:on?T.teal:T.white,color:on?"#fff":T.muted,fontSize:12,fontWeight:on?700:400,cursor:"pointer",fontFamily:"inherit",transition:"all .15s"}}>
            ...
            <tbody>
              {filtered.map((o,i)=>(
                <tr key={o.id} style={{borderTop:`1px solid ${T.border}`,background:i%2?T.bg:T.white}}>
                  ...
                  <td style={{padding:"10px 14px",display:"flex",gap:5,flexWrap:"wrap"}}>
        ...
        <Modal title={`Order ${open.id}`} onClose={()=>setOpen(null)} w={520}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:18,flexWrap:"wrap"}}>
            {["Pending","Packing","Delivering","Delivered"].map((s,i,arr)=>{
              const statuses=["Pending","Packing","Delivering","Delivered","Cancelled"];
              const curIdx=statuses.indexOf(open.status);
              const thisIdx=statuses.indexOf(s);
              const done=curIdx>=thisIdx && open.status!=="Cancelled";
              const isCurrent=open.status===s;
              return (
                <div key={s} style={{display:"flex",alignItems:"center",gap:4}}>
                  <div style={{display:"flex",alignItems:"center",gap:5,padding:"4px 10px",borderRadius:20,background:done?T.teal+"22":T.bg,border:`1.5px solid ${isCurrent?T.teal:done?T.teal+"55":T.border}`}}>
                    <span style={{width:7,height:7,borderRadius:"50%",background:done?T.teal:T.border,flexShrink:0}}/>
                    <span style={{fontSize:11,fontWeight:isCurrent?700:400,color:done?T.teal:T.muted}}>{s}</span>
                  </div>
          ...
          <div style={{background:T.bg,borderRadius:10,padding:14,marginBottom:16}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0 0",borderTop:`1px solid ${T.border}`,marginTop:4,fontWeight:800,fontSize:15}}><span>Total</span><span style={{color:T.teal}}>₵{open.subtotal+open.fee}</span></div>
          </div>

          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {next[open.status] && (
              <Btn onClick={()=>advance(open)}>Mark as {next[open.status]} →</Btn>
            )}
            {open.status!=="Cancelled"&&open.status!=="Delivered"&&(
              <Btn v="danger" onClick={()=>cancel(open)}>Cancel Order</Btn>
            )}
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:10}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <select value={stockFilter} onChange={e=>setStockFilter(e.target.value)} style={{padding:"8px 10px",borderRadius:9,border:`1.5px solid ${T.border}`,fontSize:12,fontFamily:"inherit",background:T.white,color:T.text,outline:"none",cursor:"pointer"}}>
          ...
          <div style={{display:"flex",gap:2,background:T.bg,borderRadius:9,padding:3,border:`1px solid ${T.border}`}}>
            {["grid","list"].map(v=>(
              <button key={v} onClick={()=>setView(v)} style={{padding:"5px 9px",borderRadius:7,border:"none",background:view===v?T.white:"transparent",cursor:"pointer",fontSize:13,color:view===v?T.text:T.muted,boxShadow:view===v?"0 1px 4px rgba(0,0,0,.08)":"none"}}>
                {v==="grid"?"⊞":"☰"}
              </button>
            ))}
          </div>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        <button onClick={()=>setCat("All")} style={{padding:"6px 13px",borderRadius:20,border:`1.5px solid ${catFilter==="All"?T.teal:T.border}`,background:catFilter==="All"?T.teal:T.white,color:catFilter==="All"?"#fff":T.muted,fontSize:12,fontWeight:catFilter==="All"?700:400,cursor:"pointer",fontFamily:"inherit"}}>All</button>
        {categories.map(c=>(
          <button key={c.id} onClick={()=>setCat(String(c.id))} style={{padding:"6px 13px",borderRadius:20,border:`1.5px solid ${catFilter===String(c.id)?T.teal:T.border}`,background:catFilter===String(c.id)?T.teal:T.white,color:catFilter===String(c.id)?"#fff":T.muted,fontSize:12,cursor:"pointer",fontFamily:"inherit",fontWeight:catFilter===String(c.id)?700:400}}>
            {c.icon} {c.name}
          </button>
        ))}
      </div>
        ...
        view==="grid" ? (
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))",gap:14}}>
            {shown.map(p=>{
              ...
              return (
                <Card key={p.id} style={{padding:0,overflow:"hidden",transition:"box-shadow .15s"}}>
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              ...
              <tbody>
                {shown.map((p,i)=>{
                  const c=categories.find(x=>x.id===p.catId);
                  return (
                    <tr key={p.id} style={{borderTop:`1px solid ${T.border}`,background:i%2?T.bg:T.white}}>
                      <td style={{padding:"10px 14px"}}>
                        <div style={{display:"flex",alignItems:"center",gap:10}}>
                          <div style={{width:36,height:36,borderRadius:9,background:c?c.color+"22":T.bg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0,overflow:"hidden"}}>
      ...
      {open && (
        <Modal title={editing?"Edit Product":"Add Product"} onClose={()=>{setOpen(false);setEdit(null);}}>
          <div style={{marginBottom:14}}>
            ...
            <div onClick={()=>fileRef.current.click()} style={{width:"100%",height:140,borderRadius:12,border:`2px dashed ${T.border}`,background:T.bg,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",overflow:"hidden"}}>
      ...
      {shown.length===0?<Card><EmptyState icon="🗂️" msg="No categories found"/></Card>:(
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12}}>
          {shown.map(c=>{
            ...
            return (
              <Card key={c.id} style={{position:"relative",overflow:"hidden",padding:16,transition:"box-shadow .15s"}}>
...
function Customers({customers,setCustomers,orders}) {
  ...
  const getCustOrders = cust => orders.filter(o=>o.customer===cust.name);
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:10}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <select value={statusFilter} onChange={e=>setStatusFilter(e.target.value)} style={{padding:"8px 10px",borderRadius:9,border:`1.5px solid ${T.border}`,fontSize:12,fontFamily:"inherit",background:T.white,color:T.text,outline:"none",cursor:"pointer"}}>
      ...
      {open && (
        <Modal title="Customer Profile" onClose={()=>setOpen(null)} w={500}>
            ...
            <div style={{marginBottom:14}}>
              ...
              {getCustOrders(open).map(o=>(
                <div key={o.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:`1px solid ${T.border}`,fontSize:13}}>
                  ...
                  <span style={{fontWeight:700}}>₵{o.subtotal+o.fee}</span>
                  <Tag s={o.status}/>
                </div>
              ))}
            </div>
          ...
          <div style={{display:"flex",gap:8}}>
            <Btn full v={open.status==="Active"?"danger":"primary"} onClick={()=>{setCustomers(p=>p.map(c=>c.id===open.id?{...c,status:c.status==="Active"?"Inactive":"Active"}:c));setOpen(prev=>({...prev,status:prev.status==="Active"?"Inactive":"Active"}));}}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>
        {[
          ...
          <Card key={s.label} style={{display:"flex",alignItems:"center",gap:14}}>
            <div style={{width:44,height:44,borderRadius:12,background:s.color+"1a",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>{s.icon}</div>
            <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:5}}>
              ...
              <div style={{flex:1}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:3}}>
                  <span style={{fontWeight:600}}>{m} <span style={{color:T.muted,fontWeight:400,fontSize:11}}>({n} orders)</span></span>
      ...
      <Card style={{padding:0,overflow:"hidden"}}>
        <div style={{padding:"12px 16px",borderBottom:`1px solid ${T.border}`,display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
          <span style={{fontWeight:700,fontSize:13,flex:1}}>All Transactions</span>
          <SearchBar value={search} onChange={setSearch} placeholder="Search transactions…"/>
          <select value={methodFilter} onChange={e=>setMethodFilter(e.target.value)} style={{padding:"7px 10px",borderRadius:9,border:`1.5px solid ${T.border}`,fontSize:12,fontFamily:"inherit",background:T.white,color:T.text,outline:"none",cursor:"pointer"}}>
...
function Login({onLogin}) {
  ...
  return (
    <div style={{minHeight:"100vh",background:`linear-gradient(135deg,${T.navy} 60%,${T.tealDk})`,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:T.white,borderRadius:22,padding:"38px 34px",width:"100%",maxWidth:400,boxShadow:"0 32px 80px rgba(0,0,0,.3)"}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{width:60,height:60,borderRadius:18,background:T.tealLt,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,margin:"0 auto 14px",border:`2px solid ${T.teal}33`}}>🧺</div>
...
export default function App() {
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",background:T.bg,fontFamily:"'DM Sans','Inter',system-ui,sans-serif",color:T.text}}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        tr:hover { background: #f8fffe !important; }
      `}</style>
      <Sidebar page={page} go={setPage} pendingCount={pending} collapsed={collapsed} setCollapsed={setCollapsed}/>
      <main style={{flex:1,padding:26,overflowY:"auto",minWidth:0}}>
        ...
        {page==="settings"   && <Settings   onLogout={()=>setLoggedIn(false)}/>}
      </main>
    </div>
  );
}